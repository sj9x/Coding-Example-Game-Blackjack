...
class Player(object):
    ...
    def PrintValidMoves(self):
        ...
        if self.current_hand==-1:
            return 'Type D for Deal and Q to Quit-->'
        elif self.current_hand==0:
            if self.hands[0][0].value==self.hands[0][1].value and len(self.hands)+1<3:
                if len(self.hands[self.current_hand])==2 and len(self.hands)<2:
                    return 'Type U to surrender, H to Hit, S to Stand or P to Split for Hand no.'+ str(self.current_hand+1)+ '-->'
                else:
                    return 'Type H to Hit, S to Stand or P to Split for Hand no.'+ str(self.current_hand+1)+ '-->'               
            else:
                if len(self.hands[self.current_hand])==2 and len(self.hands)<2:
                    return 'Type U to surrender, H to Hit, Or S to Stand for Hand no.'+ str(self.current_hand+1)+ '-->'               
                elif len(self.hands[self.current_hand])>=5:
                    return 'Maximum number of hits reached'
                else: 
                   return 'Type H to Hit, or S to Stand for Hand no.'+ str(self.current_hand+1)+ '-->'
        elif len(self.hands[self.current_hand])>=5:
            return 'Maximum number of hits reached'
        else:
            return 'Type H to Hit, or S to Stand for Hand no.'+ str(self.current_hand+1)+ '-->'
